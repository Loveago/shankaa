...
const OrderFiles = () => {
  ...
  const handleExportNetwork = async (network) => {
    ...
    const confirm = await Swal.fire({
      ...
      html: `<p style="color:#94a3b8">Download <b style="color:#fff">${count}</b> pending ${network} orders as Excel?</p><p style="color:#94a3b8;margin-top:8px">This will create a batch for tracking and status updates.</p>`,
      icon: 'question',
    ...
    try {
      ...
      Swal.fire({ title: 'Exported', text: `${count} ${network} orders exported successfully`, icon: 'success', background: '#1a1a2e', color: '#fff', confirmButtonColor: '#06b6d4', timer: 2000 });
      fetchBatches();
    } catch (err) {
  ...
  const handleBulkStatusUpdate = async (batchId) => {
    ...
    if (status === 'Cancelled') {
      const c = await Swal.fire({ title: 'Confirm Cancellation', text: 'This will cancel ALL orders in this batch and auto-refund the agents. Are you sure?', icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, Cancel & Refund', confirmButtonColor: '#ef4444', background: '#1a1a2e', color: '#fff' });
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-dark-400 text-sm">
                Exported: {formatDate(batchDetail.createdAt)}
              </p>
            </div>
          </div>
    ...
    <div>
      ...
      <div className="mb-4">
        <div className="relative max-w-md">
          ...
          <input type="text" placeholder="Search by batch #, network..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setPage(1); }}
