...
import { Menu, Wallet, Package, Clock, CheckCircle, ShoppingCart, Loader2, RefreshCw, Trash2, UserCircle, History, X, Banknote, Key } from 'lucide-react';
...
const NormalAgent = () => {
  ...
  const getCarrierCategory = (name) => {
    ...
    if (upper.startsWith('MTN')) return 'MTN - NORMAL';
    if (upper.startsWith('TELECEL')) return 'TELECEL - NORMAL';
    if (upper.startsWith('AIRTEL')) return 'AIRTEL TIGO - NORMAL';
    return 'OTHER';
  };

  const filteredProducts = useMemo(() => {
    const carrierMapping = [
      { prefix: 'MTN', category: 'MTN - NORMAL' },
      { prefix: 'TELECEL', category: 'TELECEL - NORMAL' },
      { prefix: 'AIRTEL', category: 'AIRTEL TIGO - NORMAL' }
    ];
  ...
  const addToCart = async (productId) => {
    ...
    const getEffectivePrice = (p) => {
      const match = p.rolePrices?.find(rp => rp.role === 'NORMAL');
                ...
                return (
                  <div key={product.id} className={`relative overflow-hidden rounded-xl sm:rounded-2xl shadow-lg bg-gradient-to-br ${cardGradient} ${product.stock === 0 ? 'opacity-75' : ''}`}>
                    ...
                    <div className="p-3 sm:p-4">
                      ...
                      <div className="flex items-baseline gap-1 mb-3 sm:mb-4"><span className="text-xs sm:text-sm text-white/70">GHS</span><span className="text-xl sm:text-2xl font-bold text-white">{(() => { const m = product.rolePrices?.find(rp => rp.role === 'NORMAL'); return (m && m.price != null && m.price >= 0) ? m.price : ((product.usePromoPrice && product.promoPrice != null) ? product.promoPrice : product.price); })()}</span></div>
